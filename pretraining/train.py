...
def train_model(args, model, checkpoint_save_prefix_path, gradient_accumulation_steps, tokenizer, 
    ...
    while effective_batch_acm < total_steps:
        ...
        loss, mlm_correct_num, tot_tokens, nxt_snt_correct_num, correct_contrastive_num, total_contrastive_num, step_tacl_loss, step_adv_loss, step_pooled_loss = \
        ...
        tacl_loss += step_tacl_loss.mean().item()
        adv_loss += step_adv_loss.mean().item()
        pooled_loss += step_pooled_loss.mean().item()
        ...
        if effective_batch_acm % args.print_every == 0 and print_valid:
            ...
            one_tacl_loss = tacl_loss / (effective_batch_acm * gradient_accumulation_steps)
            one_adv_loss = adv_loss / (effective_batch_acm * gradient_accumulation_steps)
            one_pooled_loss = pooled_loss / (effective_batch_acm * gradient_accumulation_steps)

            print ('At training steps {}, training loss is {}, TaCL loss is {}, ADV loss is {}, Pooled loss is {}, middle mlm acc is {}, contrastive acc is {}, \
                next sentence acc is {}'.format(effective_batch_acm, one_train_loss, one_tacl_loss, one_adv_loss, one_pooled_loss, middle_acc, contrastive_acc, nxt_snt_acc))
            print_valid = False
        ...
        if effective_batch_acm % args.save_every == 0 and save_valid:
            ...
            one_tacl_loss = tacl_loss / (args.save_every * gradient_accumulation_steps)
            one_adv_loss = adv_loss / (args.save_every * gradient_accumulation_steps)

            print ('At training steps {}, training loss is {}, TaCL loss is {}, ADV loss is {}, middle mlm acc is {}, contrastive acc is {}, \
                next sentence acc is {}'.format(effective_batch_acm, one_train_loss, one_tacl_loss, one_adv_loss, middle_acc, contrastive_acc, nxt_snt_acc))
            print ('Saving Model...')
            save_name = 'training_step_{}_middle_mlm_acc_{}_contrastive_acc_{}_nxt_sen_acc_{}_train_loss_{}'.format(effective_batch_acm,
                middle_acc, contrastive_acc, nxt_snt_acc, one_train_loss)
            save_valid = False
            ...
            if(train_loss <= best_train_loss):
              best_effective_batch = effective_batch_acm
    ...
    return model, best_train_loss, best_effective_batch
...
if __name__ == '__main__':
    ...
    print('batch size per gpu {}, grad_accu {}, no. of gpu {}, effective batch size {}\n'.format(batch_size_per_gpu, gradient_accumulation_steps, number_of_gpu, effective_batch_size))
    ...
    model, best_train_loss, best_effective_batch = train_model(args, model, checkpoint_save_prefix_path, gradient_accumulation_steps, tokenizer, 
    ...
    print('Best Effective Batch: ')
    print(best_effective_batch)
